...
def diff(a, b, context=None, format=None, diffs_only=False):
    """
    ...
    ARGUMENTS:
       ...
       ``diffs_only``:
         If true, returns only the INSERT and REMOVE lines. Else, return all
         lines including KEEP. 
    """
    diff = _myers(a, b)
...
def get_inserts(a, b):
    res = []
    diff_content = _myers(a, b)
    for d, e in diff_content:
        if d == INSERT:
            res.append(e.rstrip())
    return res
